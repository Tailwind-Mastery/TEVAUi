...
export default {
    ...
    methods:{
        async checkData(){
            ...
            await this.store.panel1Actions('register', 'POST', data, 'user');
            this.error = this.store.panel1.error;
            this.hideSpinner = true;
            
            if(this.error === ''){
                this.$emit('emitEvent', 'Signin')
            }
        },
    },
    emits: ['emitEvent']
...
<template>
    <div class="flex flex-col py-5 gap-5 items-center">
        ...
        <p class="text-xs text-red-500 w-44">
            **Dashboard Wont Open Without Signing Up Using Provided Data**
        </p>
        ...
        <form @submit.prevent="checkData" class="flex flex-col gap-2 items-start" :class="{'hidden': !hideSpinner}">
            ...
            <input type="password" id="password_confirm" class="border p-2 text-sm rounded" placeholder="password confirm" v-model="password_confirmation">

            <button type="submit" class="bg-cyan-500 text-white text-sm px-2 py-1 rounded mt-3">Sign Up</button>
